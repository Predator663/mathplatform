...
def _freeze_and_autofit(ws, freeze_row, freeze_col, min_widths=None):
    ...
    for col in ws.columns:
        ...
        if min_widths and col[0].column <= len(min_widths):
            adjusted = max(adjusted, min_widths[col[0].column - 1])
...
def generate_exam_scores_excel(exam, scores, sort_by='name',
    ...
    stats = [('Students', len(scores_list)), ('Present', len(present)), ('Absent', absent_count),
             ('Average', f'{avg}%'), ('Pass Rate', f'{pass_rate}%'),
             ('Highest', f'{max(pcts, default=0)}%'), ('Lowest', f'{min(pcts, default=0)}%')]
...
def generate_class_report_excel(classroom, students, scores_map, exams,
    ...
    for rank, (s, s_scores, avg) in enumerate(rows_data, 1):
        ...
        for ei, e in enumerate(exams, 4):
            ...
            c.alignment = _align('center')
            c.font = _font(size=8,
                color=GREEN[2:] if pct and pct>=75 else AMBER[2:] if pct and pct>=45 else ROSE[2:] if pct else GRAY[2:])
        ...
        avg_c.font = _font(bold=True, size=8,
            color=GREEN[2:] if avg and avg>=75 else AMBER[2:] if avg and avg>=45 else ROSE[2:] if avg else GRAY[2:])
        avg_c.border = _border()
        avg_c.alignment = _align('center')
        if avg: all_avgs.append(avg)
        next_row += 1

    # Class average footer row
    class_avg = round(sum(all_avgs)/len(all_avgs), 1) if all_avgs else 0
    ws.cell(next_row, 3, 'CLASS AVERAGE').font = _font(bold=True, size=9)
    ws.cell(next_row, ncols, f'{class_avg}%').font = _font(bold=True, size=9,
        color=GREEN[2:] if class_avg>=75 else AMBER[2:] if class_avg>=45 else ROSE[2:])
    for j in range(1, ncols+1):
        ws.cell(next_row, j).fill = PatternFill('solid', fgColor='FFD1FAE5')
        ws.cell(next_row, j).border = _border()

    _freeze_and_autofit(ws, 7, 4)
