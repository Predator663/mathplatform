...
def generate_exams_list_excel(exams, school_name='School of Excellence') -> bytes:
    """
    Styled workbook for the *exam list* itself — one row per exam, covering
    every classroom/subject/term/year it belongs to. Distinct from
    generate_exam_scores_excel(), which exports one exam's student scores.

    `exams` is any iterable of Exam instances (ideally with .present_scores
    prefetched via to_attr, and .classrooms prefetched, to avoid N+1s).
    """
    exams = list(exams)
    ...
    years = sorted({e.academic_year for e in exams if e.academic_year})
    year_label = years[0] if len(years) == 1 else (f'{years[0]}–{years[-1]}' if years else '—')

    ncols = 12
    next_row = _write_platform_header(
        ws, school_name, 'Exams Register',
        f'{len(exams)} exam{"s" if len(exams) != 1 else ""}  |  '
        f'Academic Year{"s" if len(years) != 1 else ""}: {year_label}',
        year_label, ncols,
    )

    published_count = sum(1 for e in exams if e.is_published)
    stats = [
        ('Total Exams', len(exams)),
        ('Published', published_count),
        ('Draft', len(exams) - published_count),
    ]
    next_row += 1
    for j, (label, val) in enumerate(stats, 1):
        lc = ws.cell(next_row, j, label)
    ...
    col_headers = [
        '#', 'Title', 'Subject', 'Classrooms', 'Type', 'Term', 'Year',
        'Exam Date', 'Max', 'Status', 'Avg %', 'Pass %',
    ]
    for j, h in enumerate(col_headers, 1):
        c = ws.cell(next_row, j, h)
        c.font = _font(bold=True, color=WHITE, size=9)
    ...
    # Sort newest exam first, matching the platform's default ordering.
    exams_sorted = sorted(exams, key=lambda e: e.exam_date, reverse=True)

    for rank, exam in enumerate(exams_sorted, 1):
        ...
        row_data = [
            rank, exam.title, exam.subject.name if exam.subject_id else '—',
            classroom_names, exam.get_exam_type_display(), exam.get_term_display(),
            exam.academic_year, exam.exam_date.strftime('%d %b %Y'), float(exam.max_score),
            'Published' if exam.is_published else 'Draft',
            avg / 100 if avg is not None else '—',
            pass_rate / 100 if pass_rate is not None else '—',
        ]
        for j, val in enumerate(row_data, 1):
            c = ws.cell(next_row, j, val)
            c.fill = row_fill
            c.border = _border()
            c.alignment = _align('center' if j not in (2, 3, 4) else 'left', wrap=(j in (3, 4)))
            c.font = _font(size=9)

        if avg is not None:
            ws.cell(next_row, 11).number_format = '0.0%'
            ws.cell(next_row, 11).font = _font(
                bold=True, size=9,
                color=GREEN[2:] if avg >= 75 else AMBER[2:] if avg >= 45 else ROSE[2:])
        if pass_rate is not None:
            ws.cell(next_row, 12).number_format = '0.0%'
            ws.cell(next_row, 12).font = _font(
                bold=True, size=9,
                color=GREEN[2:] if pass_rate >= 50 else AMBER[2:] if pass_rate >= 25 else ROSE[2:])

        status_cell = ws.cell(next_row, 10)
        status_cell.font = _font(bold=True, size=9,
                                  color=GREEN[2:] if exam.is_published else AMBER[2:])

        next_row += 1

    min_widths = [4, 26, 14, 22, 16, 10, 10, 12, 6, 10, 8, 8]
    _freeze_and_autofit(ws, data_start, 3, min_widths=min_widths, fixed_cols={2, 4})
