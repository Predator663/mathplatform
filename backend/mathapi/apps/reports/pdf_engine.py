...
def _compact_name(name: str) -> str:
    """
    Tanzanian names are commonly given/middle/surname (three or more
    words) — long enough to overlap neighbouring table cells or spill
    into the next row. Keeps the first and last word in full and
    ...
    Two-word (or shorter) names are returned unchanged.
    """
    if not name:
        return name
    parts = name.split()
...
def _name_cell(name: str, styles, style_key='table_name'):
    """A table-cell-safe name: initialed middle name(s) plus real word-wrap
...
def _make_styles():
    ...
    # Table-cell name styles — deliberately small/tight leading so a
    # wrapped second line doesn't blow out row height, used anywhere a
    # student/entrant name shares a row with other columns (leaderboard,
    # challenge combatants, matchup sheets) and needs to wrap instead of
    # overlapping the next cell.
    styles['table_name'] = ParagraphStyle('table_name',
        fontSize=8, fontName='Helvetica-Bold',
        textColor=BRAND_DARK, leading=9.5)

    styles['table_name_reg'] = ParagraphStyle('table_name_reg',
        fontSize=8, fontName='Helvetica',
        textColor=BRAND_DARK, leading=9.5)
...
def _header_footer(canvas, doc, meta: dict):
    ...
    left_text = f"{school}  ·  Generated by {meta.get('platform_name') or 'MathPlatform'}"
    canvas.drawString(MARGIN, 1.2*cm, left_text)

    # Right: page number + generated date
    right_text = f"Page {doc.page}  ·  Generated {date.today().strftime('%d %b %Y')}"
    canvas.drawRightString(PAGE_W - MARGIN, 1.2*cm, right_text)

    # Centre: term / exam info — only drawn if it actually fits in the gap
    # between the left and right strings. A long school name can otherwise
    # push "Generated by ..." far enough right to collide with centred text
    # (drawString/drawCentredString don't clip against each other).
    centre_text = meta.get('footer_centre', '')
    if centre_text:
        left_end = MARGIN + canvas.stringWidth(left_text, 'Helvetica', 7.5)
        right_start = PAGE_W - MARGIN - canvas.stringWidth(right_text, 'Helvetica', 7.5)
        centre_half = canvas.stringWidth(centre_text, 'Helvetica', 7.5) / 2
        gap_pad = 0.4 * cm
        if left_end + gap_pad <= PAGE_W/2 - centre_half and PAGE_W/2 + centre_half <= right_start - gap_pad:
    ...
    if results:
        ...
        for r in results:
            ...
            rows.append([
                ...
                _name_cell(r.entry.display_name, styles),
...
def generate_challenge_matchups_pdf(tournament, challenges, unmatched_entries,
    ...
    can find their own name and see their opponent(s) at a glance, plus
    who's still waiting to be matched. No scores, analytics, or classroom
    averages here on purpose; that's what the dossier is for.

    Every name goes through _name_cell so a three-part Tanzanian name
    (given + father's name + surname) wraps onto a second line inside its
    own column instead of overlapping the combatant next to it — long
    ...
    void_count = sum(1 for c in challenge_list if c.status == 'void')

    meta = {
        'school_name': school_name,
        'academic_year': tournament.exam.academic_year,
        'doc_title': f'Challenge Matchups — {tournament.title}',
        'doc_subtitle': (
            (f'Codename: {tournament.codename}  ·  ' if tournament.codename else '') +
            f'{tournament.get_mode_display()}  ·  {tournament.classroom}  ·  Who you\'re facing'
        ),
        'footer_centre': f'Classroom: {tournament.classroom}',
    }
    ...
    story.append(_meta_grid([
        ('TOTAL DUELS', len(challenge_list)),
    ...
    # ── Matchups table ────────────────────────────────────────────────────
    status_labels = {'pending': 'Awaiting Results', 'resolved': 'Resolved', 'void': 'Void'}
    if challenge_list:
        story.append(Paragraph('Matchups', styles['section']))
        rows = [['#', 'Duel', 'Combatants', 'Status']]
        for i, c in enumerate(challenge_list, 1):
            entries = list(c.entries.all())
            combatants_text = ' <b>vs</b> '.join(_compact_name(e.display_name) for e in entries)
            if c.status == 'resolved' and c.winner:
                combatants_text += f'<br/><font size="7" color="#059669">Winner: {_compact_name(c.winner.display_name)}</font>'
            elif c.status == 'resolved' and c.is_tie:
        ...
        tbl = Table(rows, colWidths=[(PAGE_W - 2*MARGIN)*p for p in [0.06, 0.24, 0.52, 0.18]], repeatRows=1)
        tbl.setStyle(TableStyle([
            ('BACKGROUND', (0, 0), (-1, 0), BRAND_DARK),
            ('TEXTCOLOR', (0, 0), (-1, 0), colors.white),
            ('FONTNAME', (0, 0), (-1, 0), 'Helvetica-Bold'),
        ...
        story.append(Paragraph('No duels declared yet.', styles['body']))
        story.append(Spacer(1, 0.4*cm))

    # ── Not yet matched ──────────────────────────────────────────────────
    if unmatched_entries:
        story.append(Paragraph('Awaiting a Challenge', styles['section']))
        story.append(Paragraph(
            'These entrants haven\'t been placed in a duel yet — check back once more matchups are declared.',
            styles['caption'],
        ))
