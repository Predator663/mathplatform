...
class DashboardSummaryView(APIView):
    ...
    def get(self, request):
        ...
        student_recent: dict = defaultdict(list)
        score_rows = list(
            ExamScore.objects.filter(score_filter)
            .filter(exam__max_score__gt=0)
            .annotate(pct=pct_expr)
            .values('student_id', 'pct', 'exam__exam_date')
            .order_by('student_id', '-exam__exam_date')
        )
        for row in score_rows:
            sid = row['student_id']
            if row['pct'] is not None and len(student_recent[sid]) < 3:
                student_recent[sid].append(row['pct'])
        at_risk_count = sum(
            1 for recent in student_recent.values()
            if recent and sum(recent) / len(recent) < 30.0
        )
