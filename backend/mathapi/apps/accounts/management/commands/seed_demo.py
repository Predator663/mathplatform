...
# Note: these are all Mathematics-subject topics. The grade band (primary vs
# o_level) used to live on a 'level' field on MathTopic, but in v2 that field
# is gone — topics belong to a Subject, and grade-band scoping happens via the
# Exam → Classroom → GradeLevel chain instead. So no 'level' marker is kept here.
OLEVEL_TOPICS = [
    ('Numbers & Operations',   '#6366f1'),
    ('Algebra',                '#f59e0b'),
    ('Geometry',                '#10b981'),
    ('Trigonometry',           '#3b82f6'),
    ('Statistics',             '#ec4899'),
    ('Vectors',                '#8b5cf6'),
    ('Coordinate Geometry',    '#06b6d4'),
    ('Matrices & Transformations', '#f97316'),
]

# ── Tanzania Primary Math Topics ───────────────────────────────────────────────
PRIMARY_TOPICS = [
    ('Nambari (Numbers)',       '#6366f1'),
    ('Maumbo (Shapes)',         '#10b981'),
    ('Kipimo (Measurement)',    '#f59e0b'),
    ('Takwimu (Statistics)',    '#3b82f6'),
    ('Sehemu (Fractions)',      '#ec4899'),
]
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # ── Subject ───────────────────────────────────────────────────────────
        # Migration 0003 already seeds this, but get_or_create here makes the
        # command safe to run standalone (e.g. against a fresh test DB).
        math_subject, _ = Subject.objects.get_or_create(
            code='MATH',
            defaults={'name': 'Mathematics', 'color': '#6366f1', 'icon': 'calculator'}
        )
        ...
        # ── Classrooms ────────────────────────────────────────────────────────
        # NOTE: Classroom.teachers (M2M) is kept only for backward compatibility.
        # Everything in v2 — ClassroomViewSet, scope_exams, MathTopicViewSet,
        # UserListView, SubjectViewSet — reads from TeacherAssignment instead.
        # Without a TeacherAssignment row, a teacher's dashboard shows nothing
        # at all (no classrooms, no students, no exams, no topics), even though
        # the M2M link exists. So every teacher↔classroom link below also gets
        # a matching TeacherAssignment(subject=math_subject).
        def assign_teacher(classroom, teacher):
            classroom.teachers.add(teacher)
            TeacherAssignment.objects.get_or_create(
                teacher=teacher, classroom=classroom, subject=math_subject
            )
        ...
        # ── Math Topics ───────────────────────────────────────────────────────
        topic_objs = {}
        for i, (name, color) in enumerate(OLEVEL_TOPICS + PRIMARY_TOPICS):
            t, _ = MathTopic.objects.get_or_create(
                subject=math_subject, name=name,
        ...
        olevel_topics_list = [topic_objs[n] for n, _ in OLEVEL_TOPICS]
        primary_topics_list = [topic_objs[n] for n, _ in PRIMARY_TOPICS]
        ...
        for title, etype, term, edate, max_s, pass_s, cls_list in exam_configs:
            # Attribute the exam to whichever teacher actually teaches these
            # classrooms (form3sci/std7 → teacher2), not always teacher1 —
            # otherwise teacher2 can never delete their own seeded exams,
            # since ExamViewSet.destroy() only allows the creator or an admin.
            owner = teacher2 if any(c in (form3sci, std7) for c in cls_list) else teacher1
