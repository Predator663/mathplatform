...
class ExamSerializer(serializers.ModelSerializer):
    ...
    def get_score_count(self, obj):
        return obj.scores.filter(is_absent=False).count()

    def get_average_score(self, obj):
        scores = obj.scores.filter(is_absent=False)
        if not scores.exists():
            return None
        total = sum(float(s.score) for s in scores)
        avg = total / scores.count()
        return round((avg / float(obj.max_score)) * 100, 1)

    def get_pass_rate(self, obj):
        scores = obj.scores.filter(is_absent=False)
        if not scores.exists():
            return None
        passed = scores.filter(score__gte=obj.passing_score).count()
        return round((passed / scores.count()) * 100, 1)
...
class ExamCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Exam
        fields = [
            'title', 'exam_type', 'term', 'academic_year', 'exam_date',
            'max_score', 'passing_score', 'classrooms', 'description',
            'is_published', 'topic_weights',
        ]
...
class BulkScoreSerializer(serializers.Serializer):
    """For bulk uploading scores for an exam."""
