...
export default function BulkImportPage() {
  const canImportStudents = useCanManage('students', 'add');
  // Bulk score import maps to the same "exams / edit" toggle as manual
  // score entry, since it's just a faster way to write exam data.
  const canImportScores = useCanManage('exams', 'edit');
  const availableModes = [
    ...(canImportStudents ? (['students'] as const) : []),
    ...(canImportScores ? (['scores'] as const) : []),
  ];
    ...
    return (
      <div className="flex flex-col gap-6 max-w-3xl page-enter">
        ...
        <div className="card">
          <EmptyState
            icon={<Upload size={32} />}
            title="Bulk import is disabled"
            message="An administrator has turned off bulk student and score imports for teacher accounts."
          />
        </div>
      </div>
    );
  ...
  return (
    <div className="flex flex-col gap-6 max-w-3xl page-enter">
      ...
      <div className="flex gap-1 bg-surface-900 p-1 rounded-xl w-fit">
        ...
        ] as { id: ImportMode; label: string; icon: typeof Users }[])
          .filter(({ id }) => availableModes.includes(id))
