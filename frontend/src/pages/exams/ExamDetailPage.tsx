    ...
    <div className="flex flex-col gap-6 page-enter">
      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-center gap-2">
          <button onClick={() => navigate('/exams')} className="text-secondary hover:text-primary text-sm transition-colors">Exams</button>
          <span className="text-secondary">/</span>
          <span className="text-sm text-primary truncate">{exam.title}</span>
        </div>
        <div className="flex items-start justify-between gap-3 flex-wrap">
          ...
          <div className="flex flex-wrap gap-2">
            <Button variant="primary" size="sm" onClick={() => navigate(`/exams/${examId}/marks`)}>
              <Edit size={13} /> <span className="hidden xs:inline">Enter </span>Marks
            </Button>
            {!exam.is_published && (
              <Button
                variant="primary"
            ...
            <Button variant="secondary" size="sm" onClick={handleExportCSV}>
              <Download size={13} /> <span className="hidden xs:inline">Export </span>CSV
            </Button>
        ...
        {editScore && (
          <div className="flex flex-col gap-4">
            <div className="bg-surface-900 rounded-xl p-3">
              <p className="text-sm font-display font-medium text-primary">{editScore.student_name}</p>
              <p className="text-xs text-secondary mt-0.5">Current score: {editScore.score}/{exam.max_score}</p>
            </div>
            <Input
              label={`New Score (max ${exam.max_score})`}
              type="number"
              min={0}
