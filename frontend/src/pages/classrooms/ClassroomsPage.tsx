...
import { School, Plus, Users, BarChart3, FileText, CloudOff } from 'lucide-react';
import { studentsApi } from '../../api';
import { LoadingPage, EmptyState, Button, Pagination } from '../../components/ui';
import { useSiteSettingsStore } from '../../store/siteSettings';
import { useCanManage } from '../../hooks/useCanManage';
import { useCachedClassrooms } from '../../hooks/useOfflineData';
...
export default function ClassroomsPage() {
  ...
  const canAdd = useCanManage('classrooms', 'add');

  const { data, isLoading, isError } = useQuery<PaginatedResponse<Classroom> | Classroom[]>({
    queryKey: ['classrooms', page, pageSize],
    queryFn: () => studentsApi.classrooms({ page, page_size: pageSize }).then(r => r.data),
    retry: 1,
  });

  // Offline (or just a failed request) — fall back to the cached snapshot
  // rather than showing an empty page.
  const cachedClassrooms = useCachedClassrooms();
  const usingOfflineData = isError;

  const online: Classroom[] = Array.isArray(data)
    ? data : (data as PaginatedResponse<Classroom>)?.results ?? [];
  const classrooms: Classroom[] = usingOfflineData ? cachedClassrooms : online;
  const total: number = usingOfflineData
    ? cachedClassrooms.length
    : (Array.isArray(data) ? data.length : (data as PaginatedResponse<Classroom>)?.count ?? 0);
    ...
    <div className="flex flex-col gap-4 md:gap-6">
      ...
      {usingOfflineData && (
        <div className="flex items-center gap-2 text-xs text-amber-400 bg-amber-500/10 border border-amber-500/20 rounded-xl px-3 py-2">
          ...
          Showing data saved on this device — reconnect for the latest classrooms.
        </div>
      )}
      ...
      {!usingOfflineData && <Pagination page={page} pageSize={pageSize} total={total} onChange={setPage} />}
    </div>
  );
}
