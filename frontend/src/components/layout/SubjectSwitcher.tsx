...
export default function SubjectSwitcher() {
  ...
  useEffect(() => {
    ...
    if (list.length > 0) {
      setSubjects(list);
      // Auto-select first subject if none selected, OR if the persisted
      // activeSubjectId no longer exists in this account's subject list
      // (e.g. re-seeded demo data, a deleted/deactivated subject, or a
      // different user logging in on the same browser). Left unchecked,
      // every subject-scoped query — including the dashboard — silently
      // filters on an ID nothing matches and comes back empty, with no
      // error to show for it.
      const stillValid = activeSubjectId != null && list.some(s => s.id === activeSubjectId);
      if (!stillValid) {
        setActiveSubject(user?.role === 'super_admin' ? null : list[0].id);
      }
    }
  }, [data]);
